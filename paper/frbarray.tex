...
\date{Unpublished post-referee working draft; repository revision 9 September 2026}

\pubyear{2020}
\journal{Unpublished working draft \hfill Repository revision 9 September 2026}
\makeatletter
\def\@oddfoot{\small Unpublished working draft\hfil}
\def\@evenfoot{\hfil\small Unpublished working draft}
\makeatother
...
\begin{abstract}
We present projected Fast Radio Burst detection rates from surveys carried out using a set of hypothetical close-packed drift-scan array telescopes. The cost efficiency of such a survey falls at least as fast as the inverse square of the survey frequency. For the modeled frequencies and source-count indices, optimal array element effective areas extend from the smallest elements to approximately 43 $\rm{m^2}$.  If the power law index of the FRB integrated source count versus fluence $\alpha = d ~ln R/d ~ln F > -1$ the most cost effective telescope layout uses individual dipole elements, which provides an all-sky field of view. If $\alpha <-1$ dish arrays are more cost effective.
...
We restrict our analysis to close-packed, drift-scan arrays. Alternatives not considered include: Large single dishes with a focal plane array of feeds--these are more expensive than close packed arrays of the same collecting area; Dilute dish arrays--these have increased processing costs, with a different signal processing system than discussed here; Telescopes with tracking systems--the added cost of the tracking system does not result in an increase in the FRB discovery rate.

\section{Projected FRB detection rates}

We consider two types of array elements: on-axis paraboloidal dishes and square aperture array tiles with a single-beam\footnote{Suppose, in the interest of increasing sky coverage, a telescope designer decided to form not one but four coherent beams, producing four signals from every tile. The three new beams would each require a new signal processing channel. Given that there are now four times as many signal processors, another option would be to increase sky coverage by making each tile smaller by a factor of two in each dimension, keeping one beam per tile, and making four times as many tiles. The two systems would have the same sky coverage, number of dipoles, signal processor channels, total collecting area and provide the same FRB rate. However, the multi-beam system would have the added cost of the additional analog beam-formers as well as substantially increased system complexity. The simpler, more cost-efficient choice is to form just one beam per tile and adjust the sky coverage using the tile size $m$. We therefore proceed with consideration of single-beam tiles, setting aside the redundant multi-beam options.} coherent analog summing network. Each tile consists of $m=1, 4, 9 ...$ dipole-like antennas. $n$ elements are close-packed to form the telescope.
We leave cylinder arrays \citep{chimesystem} to future analysis since these are phased-array-dish hybrids and should produce rates that fall between the two cases we consider.

Each element covers the same instantaneous field of view $\Omega=4\pi/G$, defined in terms of the peak antenna gain $G$ \citep{era}. The effective collecting area of each element $A_{\rm{el}}$ is related to $\Omega$ via the diffraction relation,
...
Equations 1 to 3  combine to express a scaling law for the FRB detection rate,

\begin{equation}
\begin{split}
R(>F_{min}) ={}& \frac{R_o F_o^{-\alpha}c^2}{4\pi}
\left[\frac{\mathrm{SNR}_{\mathrm{min}}\,kT_{sys}\,\tau_b}
{2\sqrt{f\tau_b}}\right]^{\alpha} \\
&{}\times A_{\mathrm{el}}^{-\alpha-1}\,
...
%\begin{subfigure}[t]{0.5\textwidth}
   \includegraphics[width=\linewidth]{ratevsaeff_alpha1.png}
%\end{subfigure}

%\begin{subfigure}[t]{0.5\textwidth}
   \includegraphics[width=\linewidth]{ratevsaeff_alpha15.png}
%  \end{subfigure}

%\begin{subfigure}[t]{0.5\textwidth}
   \includegraphics[width=\linewidth]{ratevsaeff_alpha2.png}
%\end{subfigure}
...
\caption[Two numerical solutions]{Projected FRB detection rate, for $\alpha=-1.0, -1.5, -2.0$ with three survey center frequencies: 400 MHz (red), 800 MHz (green), 1600 MHz (blue) and two element choices: Dishes (smooth curves) and Aperture Array Tiles (solid circles).   Integer steps of aperture array dimension 1x1, 2x2, 3x3, etc. are shown.
   On-axis dishes smaller than $d=5\lambda$ are not commonly used because of feed blockage inefficiency, so these rates are shown as dashed curves. We assume aperture efficiency $\eta=0.5$, giving the transition at $A_{\rm{el}}=\eta\pi(2.5\lambda)^2$. The area range extends to 60 $\rm{m^2}$ to include the high-frequency optimum. Olga Navros helped prepare this figure.}
\end{figure}

\section{Cost Model}


We divide construction costs into three components: 1) the Radiation Collector, whose cost increases with $A_{\rm{el}}$; 2) the Frequency Channelizer; and 3) the Digital Beam Former.

For dish arrays we adopt Radiation Collector cost function $C_d = D_o A_{\rm{el}}^{1.25} ~n $, a bit steeper than a linear function of dish area.
...
For signal processing we adopt the combined cost function $C_s = [S_o + r S_o\log_2(n)]~f~\nu~n$, where the first term represents frequency channelization and the second represents digital beam forming.

To set the cost coefficients $D_o$, $A_o$, and $S_o$ we use cost data for an FRB-search array of 12 six-meter dishes we built at Green Bank WV in 2019, scaled to $n \sim 1000$ elements. The notebook uses $D_o=0.029~\rm{m^{-2.5}}$, $A_o=0.067$, $S_o=2.3 \times 10^{-3}~\rm{MHz^{-1}}$, and $r=0.1$.
...
Survey                              & Freq[MHz]  & Rate [events/sky/day]       & $F_{\rm{lim}}$ [Jy.ms]                                                  \\ \hline
 ...
 Parkes 2016 \citep{parkes20160} & 1400             & $4400^{+5200}_{-3100}$       & 4                         \\
 ...
 ASKAP 2018    \citep{ascap2018}  & 1400             & $37 \pm 8 $               & 26                    	\\
...
Budgets have limits, so we fix the total telescope cost at the arbitrary value 2000 units, which yields mathematical peak rates from approximately $1.2\times10^3$ to $1.7\times10^6$ FRBs per year across the modeled configurations. These are power-law extrapolations under the stated instrumental and population assumptions. This leads to a telescope with n $\sim$ 1000 elements. Under this constraint increasing the element collecting area will entail a reduction of the number of elements. This creates a peak in the rate function $R(A_{\rm{el}})$ at which the cost efficiency is maximized.

\section{Element Effective Area}

The dependence of projected FRB detection rate on $A_{\rm{el}}$ is shown in Figure 1. The assumptions used in this plot are: $T_{\rm{sys}}= 50~\rm{K},~ \tau_b = 2~\rm{ms},~ SNR_{min}=10$ and $f=0.66$, We use the Parkes survey of \citet{parkes2018} to normalize the rate: $R_o=1700~\rm{day^{-1}}=620925~\rm{yr^{-1}}$, with 365.25 days per year and $F_o=2~\rm{Jy~ms}$. The notebook uses $k=1380.649~\rm{Jy~m^2~K^{-1}}$. This is a conservative choice since other surveys find substantially higher rates. We posted the python notebooks used to make these calculations and plots on GITHUB \footnote{\texttt{https://github.com/WVURAIL/Optimization-of-Radio-Array-Telescopes-to-Search-for-Fast-Radio-Bursts} } so others can calculate rates under different assumptions.

\section{Discussion}

For aperture arrays the optimal rate changes by approximately 0.066 per frequency doubling for $\alpha =-2$ and 0.19 per frequency doubling for $\alpha =-1$. For dish arrays the corresponding factors are approximately 0.231 ($\alpha=-2$) and 0.185 ($\alpha=-1$). The dish optima are computed over all affordable integer element counts, independently of the plotting grid. For $\alpha=-2$, their effective areas are approximately 15.13, 25.42, and 42.68 $\rm{m^2}$ at 400, 800, and 1600 MHz, respectively; for $\alpha=-1.5$, they are 9.40, 15.79, and 26.44 $\rm{m^2}$. All these factors are less than 1/4 so we find the cost efficiency of such telescopes falls with frequency with at least two powers of $\nu_s$. Once the on-sky rate versus frequency is measured, if low frequency FRBs are rare, the source spectral slope and/or scattering spectral slope may compensate for the telescope cost efficiency spectral slope. The optimum frequency can then be determined by locating the point where the combined slope is zero. Published rate data are not sufficiently precise to determine this optimum frequency. However, scattering is absent in several published FRB spectra at 400~MHz, indicating that a high throughput search below this frequency should yield FRB detections.
...
The source count index $\alpha$ determines which array type (dishes versus aperture array tiles) is most cost efficient. The critical point is near $\alpha = -1.0$. If dim FRBs are rare enough ($\alpha > -1$) the all-sky array is the most cost-efficient choice.
...
We thank the anonymous reviewer for insightful comments.
