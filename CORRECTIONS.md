...
The later manuscript's cost-model prose was also reconciled with the notebook. The
earlier text mislabeled the aperture coefficient, omitted it from the displayed values,
and described separate signal-processing coefficients that the calculation does not
use. The revised text records the implemented linear and N log N terms and their
coefficients.
...
- The redundant `amssymb` package load was removed because it conflicted with
  ...
  its bibliography.
- An unsupported table-placement option was removed because the bundled legacy MNRAS
  class printed it as literal text in the PDF.
