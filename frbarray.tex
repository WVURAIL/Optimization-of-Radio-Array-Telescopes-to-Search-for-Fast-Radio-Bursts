...
We consider two types of array elements: on-axis paraboloidal dishes and square aperture array tiles.  Each tile consists of $m=1, 4, 9 ...$ dipole-like antennas, analog summed to serve as a single array element with one coherently-formed beam on the sky. For either type, $n$ elements are close-packed to form the telescope.  For simplicity we set aside consideration of multi-beam tiles. A brief examination of this option shows no strong cost efficiency benefit.
A system in which {\it all} $m$ beams are coherently formed for each tile would provide the same FRB rate as our case $m=1$, but would have higher cost.
We also leave cylinder arrays \citep{chimesystem} to future analysis since these are phased-array-dish hybrids and should produce rates that fall between the two cases we consider.
...
We divide construction costs into three components: 1) The Radiation Collector--these cost increase with $A_{\rm{el}}$; 2) The Frequency Channelizer and 3) The Beam Former. 
...
For the Frequency Channelizer we adopt the cost function $C_s = F_o ~f ~\nu ~n$, and for the Beam Former we adopt cost function $B_o ~f ~\nu ~n ln(n)$.

To set the cost coefficients $D_o$, $A_o$, $F_o$ and $B_o$ we use cost data for an FRB-search array of 12 six-meter dishes we built at Green Bank WV in 2019.   The relative cost coefficients are
$D_o= 0.029 [\rm{m^{-2.5}}]$, $ S_o = 2.3 \times 10^{-3}\rm{~[MHz^{-1}}]$, $F_o = 0.067 (check)$ and $B_o = XXX$. 
